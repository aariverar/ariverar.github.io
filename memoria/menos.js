document.addEventListener("DOMContentLoaded", function () {
  const agregarFilaButton = document.getElementById("agregarFilaOutput");

  agregarFilaButton.addEventListener("click", function (event) {
    event.preventDefault();

    const camposAdicionales = document.getElementById("camposAdicionales2");
    const inputsGroup = document.createElement("div");
    inputsGroup.className = "inputs-group2";

    const inputIds = ["CampoOutput", "DescripcionOutput"];
    const currentInputs = document.getElementsByClassName("inputs-group2").length;
